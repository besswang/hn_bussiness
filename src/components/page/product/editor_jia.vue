  ...
  <div class="editor" v-loading="loadUse">
    ...
    <div class="form-con">
      <el-form :model="goodsForm" :rules="rules" ref="goodsForm" label-width="250px" class="ruleForm">
        ...
        <el-form-item label="数量:" prop="stocks">
          <el-col :span="12">
        ...
        <el-form-item label="重量:" prop="quality">
          <el-col :span="12">
    ...
    <el-row class="btn-con">
      <el-col :span="6">
        <template v-if="goodsId">
            <el-button type="primary" class="save-btn"  @click="issueBtn('goodsForm','ruleForm2',2)">保存</el-button>
        </template>
        <template v-else>
      ...
      <template v-if="!isShowSignHandle">
        <el-col :span="12" v-if="!spcShow">
          <el-button type="primary" class="save-btn" @click.native.prevent="setSpecMt">商品规格设置</el-button>
        </el-col>
      </template>
      <template v-else>
         <input type="hidden" :do="spcShow = true">
      </template>
    </el-row>
    <!-- 商品预览-start -->
    ...
    <transition name="slide-fade">
      <template v-if="spcShow">
         <shop-spec :category-id="(categoryId || -1)" :goods-id="(goodsId || -1)"></shop-spec>
      </template>
    </transition>
  </div>
</template>
...
<script>
import shopSpec from '../../common/shopSpec.vue'
...
export default {
  ...
  created () {
      ...
      if(routeClassify){//商品发布-填写商品基本信息
        ...
        this.categoryId = this.$route.query.categoryId//获取路由传参三级目录id
  ...
  methods: {
    setSpecMt () {
      if(parseInt(this.goodsId) > 0){
        this.spcShow = !this.spcShow;
      }else{
        this.$message({
          message:'商品未发布',
          type:'warning'
    ...
    listGoodsAttributesFn(){
      let _this = this
      this.goodsId = this.$route.query.goodsId;//从商品列表--编辑-传过来的商品id
      if(this.goodsId){
      //获取已选属性-start
        ...
        .then(function(response) {
          if(response.data.code == "SUCCESS"){
            ...
            _this.categoryId = r.goods.categoryId
      ...
      //获取已选属性-end
          ...
          this.$axios.post(this.$$jk.listGoodsAttributes,{categoryId:this.categoryId})
              .then(function(response) {
                if(response.data.code == "SUCCESS"){
                  _this.goodAtrArr = response.data.data
                  _this.goodAtrArr.forEach(item => {
                      _this.goodsAttributesId.push(item.goodsAttributes.id)//原始的
                  })
                }
              })
              .catch(function(error) {
        ...
        this.$axios.post(this.$$jk.goodsRelease,goods)
          .then(function(response) {
              if(response.data.code == "SUCCESS"){
                  ...
                  _this.goodsId = response.data && response.data.data;
              }else{
